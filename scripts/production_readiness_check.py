    ...
    def check_required_packages(self):
        ...
        for package, import_name in required_packages.items():
            try:
                ...
                self.check_item(f"パッケージ: {package}", False, "インストールされていません", critical=True)
...
>>>>>>> 9ead59c0c8153a0969ef2e94b492063a605db31f
        ...
        for filepath in core_files:
            path = Path(filepath)
            self.check_item(
                f"ファイル: {filepath}", path.exists(), "見つかりません" if not path.exists() else "", critical=True
            )
...
>>>>>>> 9ead59c0c8153a0969ef2e94b492063a605db31f
                ...
                self.check_item("paper_trading.db", False, f"エラー: {e}", critical=True)
        else:
            self.check_item("paper_trading.db", False, "初回起動時に作成されます", critical=False)
        ...
        if config_path.exists():
            try:
                ...
                for key in required_keys:
                    self.check_item(
                        f"設定項目: {key}", key in config, "設定項目が不足しています" if key not in config else ""
                    )
...
>>>>>>> 9ead59c0c8153a0969ef2e94b492063a605db31f
                ...
                self.check_item("config.json", False, "JSONフォーマットエラー", critical=True)
            except Exception as e:
...
>>>>>>> 9ead59c0c8153a0969ef2e94b492063a605db31f
                ...
                self.check_item(f"モジュール: {module_name}", False, f"エラー: {e}", critical=True)
    ...
    def check_security(self):
        ...
        if gitignore_path.exists():
            ...
            for pattern in important_ignores:
                self.check_item(
                    f".gitignore: {pattern}", pattern in content, "追加を推奨" if pattern not in content else ""
                )
        else:
            self.check_item(".gitignore", False, "作成を推奨")
...
>>>>>>> 9ead59c0c8153a0969ef2e94b492063a605db31f
        ...
        for doc in docs:
            path = Path(doc)
            self.check_item(f"ドキュメント: {doc}", path.exists(), "見つかりません" if not path.exists() else "")
