        ...
        for target_col in target_cols:
            if target_col in df_out.columns:
                ...
                if abs(correlation) > 0.95:  # 閾値は調整可能
                    logger.warning(f"Possible data leakage detected: {col} with {target_col}, correlation: {correlation:.4f}")
...
if __name__ == "__main__":
    ...
    for i, (train, test) in enumerate(splits):
        print(f"Split {i+1}: Train from {train.index[0]} to {train.index[-1]} ({len(train)} samples), "
