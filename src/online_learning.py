...
class OnlineLearner:
    ...
    def __init__(self, base_model, update_frequency: str = 'daily', 
                 decay_rate: float = 0.95, performance_threshold: float = 0.95):
        """
        Args:
    ...
    def incremental_fit(self, X_new: pd.DataFrame, y_new: pd.Series, 
    ...
    def _check_performance_degradation(self) -> bool:
        ...
        if recent_avg < past_avg * self.performance_threshold:
            logger.warning(f"Performance degradation detected: {recent_avg:.4f} < {past_avg:.4f} * {self.performance_threshold}")
            return True
