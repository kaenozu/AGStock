...
def render_market_scan_tab(ticker_group, selected_market, custom_tickers, period, strategies, 
                          allow_short, position_size, enable_fund_filter, max_per, max_pbr, min_roe, trading_unit):
    """
    市場スキャンタブのレンダリングロジック
    """
    import json
    ...
    from src.paper_trader import PaperTrader
    from src.ui_components import display_error_message, display_sentiment_gauge, display_best_pick_card
        ...
        with st.spinner("データを取得し、全戦略をバックテスト中..."):
            ...
            if not results_df.empty:
                ...
                def handle_best_pick_order_fresh(ticker, action, price):
                    pt = PaperTrader()
                    trade_action = "BUY" if "BUY" in action else "SELL"
                    if pt.execute_trade(ticker, trade_action, trading_unit, price, reason=f"Best Pick: {best_strat_name}"):
                        st.balloons()
                        st.success(f"{best_pick['Name']} を {trading_unit}株 {trade_action} しました！")
                    else:
                        display_error_message(
