...
class HierarchicalStrategy(Strategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.Series:
        ...
        macd_cross_up = (df_mtf['MACD'] > df_mtf['MACD_Signal']) & (df_mtf['MACD'].shift(1) <= df_mtf['MACD_Signal'].shift(1))
        macd_cross_down = (df_mtf['MACD'] < df_mtf['MACD_Signal']) & (df_mtf['MACD'].shift(1) >= df_mtf['MACD_Signal'].shift(1))
