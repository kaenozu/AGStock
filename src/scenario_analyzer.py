        ...
        for event_name, event_info in self.historical_events.items():
            ...
            if historical_event:
                similarity_score = self._calculate_similarity(
                    current_return_30d, current_volatility,
                    historical_event['event_return'], historical_event['volatility_during_event']
                )
...
class MonteCarloSimulator:
    ...
    def simulate_portfolio(
        ...
        for t in range(1, time_steps + 1):
            ...
            # 各資産価格の変化
            for i in range(n_assets):
                dS = expected_returns[i] * asset_paths[:, i, t-1] * dt + \
...
class ScenarioBasedPredictor:
    ...
    def predict_with_scenarios(
        ...
        if hasattr(self.base_predictor, 'predict'):
            ...
            base_predictions = self.base_predictor.predict([base_features]) if len(base_features) > 0 else [0.0] * prediction_days
        else:
    ...
    def _assess_scenario_risks(self, current_data: pd.DataFrame, mc_analysis: Dict) -> Dict[str, float]:
        ...
        return {
            ...
            'var_warning_level': mc_analysis['var_5p'] / mc_analysis['mean_final_price'] if mc_analysis['mean_final_price'] > 0 else 0.0
        }
    ...
    def run_portfolio_scenario_analysis(
        ...
        stress_results = self.stress_tester.portfolio_stress_test(
            portfolio_weights=weights_array,
            asset_returns=returns_df.values
        )
