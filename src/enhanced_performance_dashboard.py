...
def create_performance_dashboard():
    ...
    with col_dist1:
        st.markdown("**ポートフォリオ**")
        fig_hist1 = go.Figure(data=[go.Histogram(
        ...
        # 統計
        st.caption(f"平均: {portfolio_returns_aligned.mean()*100:.2f}% | 標準偏差: {portfolio_returns_aligned.std()*100:.2f}%")
    ...
    with col_dist2:
        ...
        # 統計
        st.caption(f"平均: {benchmark_returns_aligned.mean()*100:.2f}% | 標準偏差: {benchmark_returns_aligned.std()*100:.2f}%")
