...
def render_past_decisions():
    ...
    for l in recent_lessons:
        with st.expander(f"{l['timestamp'][:16]} | {l['ticker']} | {l['decision']} -> {l['outcome']}"):
            col1, col2 = st.columns(2)
            with col1:
                st.write(f"**当初価格:** ¥{l['initial_price']:,.1f}")
                st.write(f"**1週間後価格:** ¥{l['price_1w']:,.1f}" if l['price_1w'] else "未更新")
            with col2:
                ret = l['return_1w']
                if ret is not None:
...
def _render_filing_watcher_ui():
    ...
    if st.session_state.filing_watcher_running:
        ...
        if st.button("🟢 監視を開始", type="primary"):
            ...
            st.rerun()
        st.info("監視を開始すると、バックグラウンドでのチェックが有効になります（現在の実装ではこのタブを表示している間、または明示的なトリガーで実行されます）")
...
def render_executive_control():
    ...
    with col1:
        ...
        arena = StrategyArena()
        data = arena.get_weights() or {"MarketAnalyst": 1.0, "RiskManager": 1.0, "MacroStrategist": 1.0}
    ...
    perf_df = pd.DataFrame([
        ...
        {"Portfolio": "積極型 (Aggressive)", "Performance": twin_perf["AGGRESSIVE"]},
        {"Portfolio": "保守型 (Conservative)", "Performance": twin_perf["CONSERVATIVE"]}
    ])
    
    fig_twin = px.bar(perf_df, x="Portfolio", y="Performance", color="Portfolio", 
    ...
    if actions:
        for act in actions:
            with st.expander(f"【{act['action']}】 {act.get('ticker', '全体')} - {act['reason'][:50]}..."):
                ...
                st.button(f"実行を承認 ({act['ticker']})", key=f"exec_{act['ticker']}_{act['action']}")
    else:
    ...
    if tax_actions:
        for t in tax_actions:
            st.info(f"💡 **損出しの提案**: {t['ticker']} で利益を相殺し、約 **¥{t['estimated_tax_savings']:,.0f}** の節税が可能です。")
    ...
    mock_news = [
        ...
        {"title": "半導体セクターに強い買い、米テック株高を受け", "published": "30分前"}
    ]
