    ...
    if cached_results and not run_fresh:
        ...
        if not results_df.empty:
            ...
            if not actionable_df.empty:
                ...
                # Ask AI Button
                if st.button(f"🤖 この銘柄 ({best_pick['Name']}) についてAIに聞く", key=f"ask_ai_{best_pick['Ticker']}"):
                    st.session_state["chat_initial_input"] = f"{best_pick['Name']} ({best_pick['Ticker']}) の詳細な分析をお願いします。なぜこの戦略が推奨されたのですか？"
