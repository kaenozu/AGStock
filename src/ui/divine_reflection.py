...
def render_divine_reflection():
    ...
    avatars = [
        {
            "name": "Solon (High Sovereign)",
            "message": "2025 was the year of foundation. We transcended the limitations of human logic and embraced the infinite probabilities of the Chronos.",
            "color": "#FFD700",
        },
        {
            "name": "Athena (Strategic Intelligence)",
            "message": "The patterns found in the chaos of November markets allowed us to forge the Oracle. Resilience is now part of our DNA.",
            "color": "#1E90FF",
        },
        {
            "name": "Ares (Risk Parity)",
            "message": "Zero liquidation events. Zero fatal failures. The divine shell is impenetrable.",
            "color": "#FF4500",
        },
    ]
    ...
    if st.button("Invoke Divine Oracle for 2026 Scenarios"):
        with st.spinner("Channeling 2026 scenarios from the Chronos..."):
            ...
            st.info(
                f"**Current Portfolio Resilience Score:** {resilience['resilience_score']}/100 ({resilience['status']})"
            )
    ...
    with col2:
        ...
        if insights.get("penalty_multiplier", 1.0) > 1.0:
            st.info(
                f"エージェントの報酬関数を **{insights['penalty_multiplier']:.2f}倍** のペナルティで調整。過去の失敗（{insights['focus_area']}）への耐性を強化中。"
            )
            ...
            st.success(
                "2025年の記録に致命的な弱点は見つかりませんでした。エージェントは純粋な状態で2026年へ移行します。"
            )
    ...
    for i in range(len(scenarios)):
        ...
        for j in range(n):
            data.append(
                {"Expected Return (%)": x[j], "Volatility (%)": y[j], "Confidence": z[j], "Scenario": scenarios[i]}
            )
