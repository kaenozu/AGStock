...
def render_earnings_analyst():
    ...
    st.markdown(
        "決算短信(PDF)をアップロードすると、AIが瞬時に分析し、投資判断を提供します。"
    )
...
def _render_new_analysis():
    ...
    with col1:
        use_rag = st.checkbox(
            "RAG（高度な検索）を使用",
            value=True,
            help="ベクトル検索で関連情報を抽出します",
        )
    with col2:
        extract_tables = st.checkbox(
            "テーブルを抽出", value=True, help="財務データのテーブルを抽出します"
        )
        ...
        if st.button("🚀 AI分析を開始", type="primary", use_container_width=True):
            with st.spinner("AIが資料を精読中... (これには数秒〜1分程度かかります)"):
                try:
                    ...
                    pdf_data = pdf_loader.load_pdf(
                        uploaded_file, extract_tables=extract_tables
                    )

                    if not pdf_data.get("text"):
                        st.error(
                            "❌ テキスト抽出に失敗しました。画像ベースのPDFの可能性があります。"
                        )
                        return
                    ...
                    st.success(
                        f"📊 企業: {metadata.get('company', '不明')} | 日付: {metadata.get('date', '不明')}"
                    )
                    ...
                    if use_rag:
                        ...
                        if success:
                            ...
                            st.warning(
                                "⚠️ RAGインデックス化に失敗しました。通常モードで続行します。"
                            )
                            rag_engine = None
...
def _render_history():
    ...
    try:
        ...
        if selected_company != "すべて":
            items = [
                item for item in items if item.get("company_name") == selected_company
            ]
        ...
        for item in items:
            ...
            with st.expander(
                f"{icon} {company} ({period}) - スコア: {score}/100 - {timestamp}"
            ):
...
def _render_settings():
    ...
    st.markdown("### API設定")
    api_key_status = (
        "✅ 設定済み" if st.session_state.get("GEMINI_API_KEY") else "❌ 未設定"
    )
...
def _display_analysis_result(result: dict, pdf_data: dict):
    ...
    with col3:
        ...
        sent_emoji = {"POSITIVE": "😊", "NEUTRAL": "😐", "NEGATIVE": "😞"}.get(
            sentiment, "😐"
        )
