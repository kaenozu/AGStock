...
class PerformanceAnalyzer:
    ...
    def analyze_performance_bottlenecks(self) -> Dict:
        ...
        if cpu_percent > self.thresholds['cpu_critical']:
            ...
            recommendations.append('CPU使用率が非常に高いです。プロセスの最適化またはスケールアップを検討してください。')
        ...
        if memory_percent > self.thresholds['memory_critical']:
            ...
            recommendations.append('メモリ使用率が非常に高いです。メモリリークの可能性があります。アプリケーションを再起動してください。')
            ...
            recommendations.append('メモリ使用率が高いです。キャッシュサイズの調整や不要なデータのクリアを検討してください。')
        ...
        if disk_percent > self.thresholds['disk_critical']:
            ...
            recommendations.append('ディスク使用率が非常に高いです。ストレージのクリーンアップまたはスケールアップが必要です。')
            ...
            recommendations.append('ディスク使用率が高いです。不要なファイルの削除やログのローテーションを検討してください。')
        ...
        if top_cpu and top_cpu[0]['cpu_percent'] > 50.0:
            ...
            recommendations.append(f"プロセス '{top_cpu[0]['name']}' がCPUを大量に使用しています。処理の最適化を検討してください。")
        ...
        if top_memory and top_memory[0]['memory_percent'] > 30.0:
            ...
            recommendations.append(f"プロセス '{top_memory[0]['name']}' がメモリを大量に使用しています。メモリ使用量の最適化を検討してください。")
        
        status = 'critical' if any(b['severity'] == 'critical' for b in bottlenecks) else \
                'warning' if bottlenecks else 'normal'
...
"""
        ...
        if trends['status'] == 'success':
            for resource, data in trends['trends'].items():
                trend_icon = "↗️" if data['trend'] == 'increasing' else \
                            "↘️" if data['trend'] == 'decreasing' else "➡️"
