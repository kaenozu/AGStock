...
class FundamentalAnalyst(Agent):
    ...
    def vote(self, ticker: str, data: Dict[str, Any]) -> AgentVote:
        ...
        return AgentVote(self.name, decision, abs(score - 0.5) * 2, f"Sentiment: {sentiment}. {reasoning}")
...
class PortfolioManager(Agent):
    ...
    def make_decision(self, ticker: str, votes: List[AgentVote]) -> Dict[str, Any]:
        ...
        for vote in votes:
            ...
            summary_lines.append(f"{icon} **{vote.agent_name}**: {vote.decision} (Conf: {vote.confidence:.2f}) - {vote.reasoning}")
