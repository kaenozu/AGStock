...
class PortfolioManager(BaseManager):
    ...
    def check_new_position(
        ...
        if self.sector_map and ticker in self.sector_map:
            ...
            sector_exposure = self._calculate_sector_exposure(sector, current_portfolio)
    ...
    def _calculate_sector_exposure(self, sector: str, portfolio: List[str]) -> float:
        """セクター露出度を計算"""
        if not portfolio:
            return 0.0

        sector_count = sum(1 for t in portfolio if self.sector_map.get(t) == sector)
...
if __name__ == "__main__":
    # テスト
    constraints = PortfolioConstraints(max_correlation=0.7, max_sector_exposure=0.3)
