...
class DynamicRiskManager:
    ...
    def update_parameters(self, df: pd.DataFrame, lookback: int = 20) -> Dict[str, Any]:
        ...
        logger.info(f"Stop loss: {self.current_params['stop_loss']:.4f}, "
    ...
    def get_position_size(self, account_balance: float, risk_per_trade: float = 0.02,
                         current_price: float = None, stop_loss_price: float = None) -> float:
        """
        動的ポジションサイズを計算
    ...
    def calculate_stop_loss(self, entry_price: float, direction: str = 'long') -> float:
        ...
        logger.info(f"Stop loss price for {direction}: {stop_loss_price:.2f} "
    ...
    def calculate_take_profit(self, entry_price: float, direction: str = 'long') -> float:
        ...
        logger.info(f"Take profit price for {direction}: {take_profit_price:.2f} "
                   f"({take_profit_pct*100:.2f}% from entry)")
        
        return take_profit_price
    
    def should_enter_trade(self, signal_strength: float = 0.7, 
                          min_strength: float = 0.6) -> bool:
        """
        トレードに入るべきか判断
        ...
        logger.info(f"Trade decision: {'ENTER' if should_enter else 'SKIP'} "
