...
def get_user_friendly_message(exception: Exception, context: str = "") -> dict:
    ...
    return {
        ...
        'technical_details': f"{context}: {type(exception).__name__}: {str(exception)}" if context else f"{type(exception).__name__}: {str(exception)}",
        'context': context
    }
...
def api_retry(func: Callable) -> Callable:
    ...
    return retry(
        ...
        delay=2.0,
        backoff=2.0,
        exceptions=(Exception,)  # 広範囲のエラーをリトライ
    )(func)
