...
def render_tax_calculator():
    ...
    with tab3:
        ...
        if positions.empty:
            ...
            st.dataframe(positions[['ticker', 'quantity', 'entry_price', 'current_price', 'unrealized_pnl']], 
            ...
            if st.button("年末戦略を分析", key="year_end"):
                ...
                if strategy.get('loss_harvesting_candidates'):
                    ...
                    for rec in strategy['loss_harvesting_candidates']:
                        st.write(f"- **{rec['ticker']}**: 損失 ¥{rec['unrealized_loss']:,.0f} → 節税効果 ¥{rec['tax_benefit']:,.0f}")
                else:
