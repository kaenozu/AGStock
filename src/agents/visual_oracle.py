...
class VisualOracle:
    ...
    def _generate_candlestick_chart(self, ticker: str, df: pd.DataFrame) -> Optional[str]:
        ...
        try:
            ...
            ax.bar(
                prices_up.index,
                prices_up.Close - prices_up.Open,
                width,
                bottom=prices_up.Open,
                color="g",
            )
            ax.bar(
                prices_up.index,
                prices_up.High - prices_up.Close,
                width2,
                bottom=prices_up.Close,
                color="g",
            )
            ax.bar(
                prices_up.index,
                prices_up.Low - prices_up.Open,
                width2,
                bottom=prices_up.Open,
                color="g",
            )
            ...
            ax.bar(
                prices_down.index,
                prices_down.Open - prices_down.Close,
                width,
                bottom=prices_down.Close,
                color="r",
            )
            ax.bar(
                prices_down.index,
                prices_down.High - prices_down.Open,
                width2,
                bottom=prices_down.Open,
                color="r",
            )
            ax.bar(
                prices_down.index,
                prices_down.Low - prices_down.Close,
                width2,
                bottom=prices_down.Close,
                color="r",
            )
