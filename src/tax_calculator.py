...
class TaxCalculator:
    ...
    def calculate_capital_gains_tax(self, 
                                   profit: float,
                                   is_nisa: bool = False) -> Dict[str, float]:
        """
        株式譲渡所得税計算
    ...
    def calculate_loss_offset(self,
                             gains: List[float],
    ...
    def optimize_loss_harvesting(self,
                                positions: pd.DataFrame,
                                target_loss: float = 0.0) -> List[Dict]:
        """
        税務損失収穫の最適化
        ...
        for _, pos in loss_positions.iterrows():
            ...
            recommendations.append({
                ...
                'tax_benefit': loss * (self.CAPITAL_GAINS_TAX_RATE + 
    ...
    def calculate_year_end_tax_strategy(self,
                                       realized_gains: float,
                                       unrealized_positions: pd.DataFrame) -> Dict:
        """
        年末税務戦略計算
        ...
        loss_harvest = self.optimize_loss_harvesting(
            ...
            target_loss=realized_gains  # 利益を相殺
        )
