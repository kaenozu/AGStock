...
from src.strategies import LightGBMStrategy, MLStrategy, CombinedStrategy, DividendStrategy
...
class MarketScanner:
    ...
    """
    def __init__(self, config: dict, paper_trader, logger, advanced_risk, asset_selector, position_manager, kelly_criterion, risk_manager):
        self.config = config
        ...
        self.asset_config = self.config.get("assets", {
            "japan_stocks": True, "us_stocks": True, "europe_stocks": True, "crypto": False, "fx": False
        })
            ...
            for strategy_name, strategy in strategies:
                try:
                    ...
                    if last_signal == 1 and not is_held and allow_buy:
                        ...
                        allow_corr, corr_reason = self.advanced_risk.check_correlation(ticker, existing_tickers, self.logger)
                        if not allow_corr:
                        ...
                        try:
                            ...
                            if "error" not in future_result:
                                ...
                                if predicted_change_pct < 0.5:
                                    self.logger.info(f"  {ticker}: 中期予測が弱い({predicted_change_pct:+.1f}%)ためスキップ")
                                    continue
                                else:
                        ...
                        try:
                            ...
                            if not history.empty and 'realized_pnl' in history.columns:
                                ...
                                if len(closed_trades) >= 10:  # Need at least 10 trades for meaningful stats
                                    ...
                                    self.logger.info(f"📊 実績ベース Kelly: 勝率={win_rate:.1f}, 損益比={win_loss_ratio:.2f} (過去{len(closed_trades)}件)")
                                else:
                                    # Not enough history, use conservative defaults
                                    win_rate = 0.50  # More conservative than 55%
                                    win_loss_ratio = 1.5
                                    self.logger.info(f"📊 デフォルト Kelly: 勝率={win_rate:.1f}, 損益比={win_loss_ratio:.2f} (履歴不足)")
                            else:
                                win_rate = 0.50
                                win_loss_ratio = 1.5
                                self.logger.info(f"📊 デフォルト Kelly: 勝率={win_rate:.1f}, 損益比={win_loss_ratio:.2f} (履歴なし)")
                        except Exception as e:
                        ...
                        if is_us_stock:
                            ...
                            if quantity < 1:
                                ...
                                if cash >= latest_price:
                                    ...
                                    self.logger.info(f"  {ticker}: 資金不足のためスキップ (必要: {latest_price:.2f}, 保有: {cash:.2f})")
                                    continue
                        else:
                            # 日本株は100株単位
