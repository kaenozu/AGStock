...
class PortfolioManager:
    ...
    def calculate_risk_parity_weights(self, tickers: List[str], price_history: Dict[str, pd.DataFrame]) -> Dict[str, float]:
        """
        ...
        for i in range(len(tickers)):
            for j in range(i + 1, len(tickers)):
                ...
                if corr > self.max_correlation:
                    suggestions.append(f"Warning: {t1} and {t2} are highly correlated ({corr:.2f}). Consider reducing one.")
