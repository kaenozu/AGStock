    ...
    def store_sentiment_data(self, sentiment_data: List[Dict]):
        ...
        for item in sentiment_data:
            ...
            ''', (
                ...
                'positive' if item['sentiment_score'] > 0.1 else 'negative' if item['sentiment_score'] < -0.1 else 'neutral'
            ))
...
class SocialMediaSentimentAnalyzer:
    ...
    def get_social_sentiment_score(self, ticker: str, hours: int = 24) -> Dict[str, float]:
        ...
        if total_engagement > 0:
            volume_adjusted_sentiment = (df['sentiment_score'] * (df['retweets'] + df['likes'] + 1)).sum() / total_engagement
        else:
...
class AlternativeDataIntegrator:
    ...
    def analyze_volume_sentiment(self, ticker: str, period: str = "5d") -> Dict[str, float]:
        ...
        try:
            ...
            latest_momentum = data['Volume'].iloc[-5:].mean() / data['Volume'].iloc[-20:-5].mean() if len(data) > 25 else 1.0
    ...
    def integrate_all_sentiment(self, ticker: str) -> Dict[str, Any]:
        ...
        overall_sentiment = (
            ...
            np.mean([item['score'] for sublist in search_trends.values() for item in sublist]) / 50.0 * weights['search']  # 検索トレンドの正規化
        )
        ...
        return {
            ...
            'sentiment_components': {
                ...
                'search': np.mean([item['score'] for sublist in search_trends.values() for item in sublist]) / 50.0 * weights['search']
            }
        }
    ...
    def get_sentiment_features(self, ticker: str, lookback_days: int = 7) -> Dict[str, float]:
        ...
        if not historical_news.empty:
            ...
            features['sentiment_momentum'] = (historical_news['sentiment_score'].iloc[-1] - 
                                            historical_news['sentiment_score'].iloc[-3:].mean()) if len(historical_news) >= 3 else 0.0
...
class SentimentEnhancedPredictor:
    ...
    def predict_with_sentiment(self, ticker: str, base_features: np.ndarray) -> Dict[str, Any]:
        """センチメント情報を統合した予測"""
        # 基本的な数値予測
        base_prediction = self.base_predictor.predict(base_features) if hasattr(self.base_predictor, 'predict') else [0.01] * 5  # 5日分の予測
        ...
        for i, pred in enumerate(base_prediction):
            ...
            adjustment_factor = self.sentiment_influence_weight * overall_sentiment * (1.0 - 0.1 * i)  # 時間とともに影響を小さく
