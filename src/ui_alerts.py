...
def render_alert_list(manager: AlertManager):
    ...
    for alert in alerts:
        with st.expander(f"{'🔔' if alert.enabled else '🔕'} {alert.message or f'{alert.ticker} {alert.condition} {alert.threshold}'}"):
...
def render_create_alert(manager: AlertManager):
    ...
    if alert_type == AlertType.PRICE.value:
        ...
        condition = st.selectbox(
            "条件",
            [AlertCondition.ABOVE.value, AlertCondition.BELOW.value]
        )
