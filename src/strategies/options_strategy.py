...
class OptionsEngine:
    ...
    def black_scholes(self, S: float, K: float, T: float, sigma: float, option_type: str = "put") -> Dict[str, float]:
        ...
        """
        if T <= 0:
            return {"price": max(0.0, (K - S) if option_type == "put" else (S - K)), "delta": 0.0}
        ...
        return {
            ...
            "theta": -(S * norm.pdf(d1) * sigma) / (2 * math.sqrt(T)) - self.r * K * math.exp(-self.r * T) * norm.cdf(d2 if option_type == "call" else -d2)
        }
    ...
    def _generate_advice_text(self, vix: float, cost: float, cost_pct: float) -> str:
        if vix > 30:
            ...
            return f"ボラティリティが上昇傾向にあります。念のため、一部ポジションの利確またはプットオプションでの保険を検討してください。"
        else:
