...
def _show_market_status_mini():
#     """market status in a mini view"""
now = datetime.now()
    weekday = now.weekday()
    if weekday < 5:
        st.write("🇯🇵 **TSE**: 営業中" if 9 <= now.hour < 15 else "🇯🇵 **TSE**: 休場中")
        st.write("🇺🇸 **NYSE**: 営業中" if 14 <= now.hour < 21 else "🇺🇸 **NYSE**: 休場中")
    else:
        st.write("🚫 **Markets**: 休場日")
def _go_no_go_mini():
    pass
#     """health check in a mini view"""
from src.utils.health import quick_health_check
health = quick_health_check()
    st.write("💻 **System**: OK" if health.get("disk_ok") and health.get("memory_ok") else "⚠️ **System**: Check")
    st.write("🌐 **API**: OK" if health.get("api_ok") else "⚠️ **API**: Issue")
...
# --- Command Center (Unified Executive View) ---
with st.container():
        pass
#         st.write("""" 🎮 Command Center (統合司令部)")
#         col1, col2, col3, col4 = st.columns(4)
#             with col1:
    pass
#                 st.write("**📡 Market Status**")
# _show_market_status_mini()
#             with col2:
    pass
...
#             with col3:
    pass
#                 st.write("**✨ Transcendent Pulse**")
#             st.metric("Score", "125.5/100", "Divine")
#             st.caption("On-Chain & Multiversal: OK")
#             with col4:
    pass
#                 st.write("**⚙️ Scenario**")
#             scenario = st.session_state.get("scenario", os.getenv("TRADING_SCENARIO", "neutral"))
#             st.metric("Mode", scenario.upper())
#             st.caption(f"Risk: {scenario}")
#         st.divider()
# # ポートフォリオ概要
#     with st.expander("💼 Portfolio & Performance (資産状況)", expanded=True):
    pass
#         _show_portfolio_summary()
# _show_performance_chart()
# # ミニカード
#     with st.expander("✨ Highlights (主要統計)", expanded=False):
    pass
#         _show_stat_cards()
# # 日次サマリー
#     with st.expander("📅 Daily Summary (当日サマリー)", expanded=False):
    pass
#         _show_daily_summary()
#         with st.expander("🔍 Risk & Model Versions (分析詳細)", expanded=False):
    pass
#             c1, c2 = st.columns(2)
#         with c1:
    pass
...
# # 通知フック
#     with st.expander("🔔 Notification Settings (通知設定)", expanded=False):
    pass
#         _notification_hooks()
