...
st.subheader("🌀 Google Gemini API")
        ...
        display_gemini = (
            ...
            st.warning("⚠️ 未設定")
            new_gemini_key = st.text_input("Gemini API Key", type="password", placeholder="AIzaSy...", key="gemini_input")
...
with tab2:
        ...
        line_config = notifications.get("line", {})
            ...
            "LINE Notify トークン", value=line_config.get("token", ""), type="password", disabled=not enable_line
        )
...
st.success("✅ 通知設定を保存しました！")
        ...
        st.caption("神託やシグナルをDiscordチャンネルに配信します。")
            discord_config = notifications.get("discord", {})
        enable_discord = st.checkbox("Discord通知を有効にする", value=discord_config.get("enabled", False))
            discord_url = st.text_input(
            "Webhook URL",
            value=discord_config.get("webhook_url", ""),
            type="password",
            disabled=not enable_discord,
            placeholder="https://discord.com/api/webhooks/...",
        )
        st.markdown("[Webhook作成方法](https://support.discord.com/hc/ja/articles/228383668)")
            if st.button("Discord設定を保存"):
                new_discord_config = {"enabled": enable_discord, "webhook_url": discord_url}
            if "notifications" not in current_config:
                current_config["notifications"] = {}
            current_config["notifications"]["discord"] = new_discord_config
_save_full_config(current_config)
