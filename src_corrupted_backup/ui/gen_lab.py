...
st.divider()
    st.header("🧬 Genetic Strategy Breeder")
    st.caption("既存の優秀な戦略同士を『交配』させ、互いの長所を持った新しいアルゴリズムを誕生させます。")
        if st.button("🧪 戦略の交配計画を実行する", type="secondary"):
            from src.evolution.genetic_breeder import GeneticBreeder
            breeder = GeneticBreeder()
        with st.spinner("AIが遺伝子（コード）の交配を開始しました..."):
            new_path = breeder.run_evolution_cycle()
            if new_path:
                st.success(f"🎊 新しい子戦略が誕生しました！: {new_path}")
                st.info("Market Simulator によるストレステストをクリアしました。")
            else:
                st.warning(
                    "現在、交配に適した十分な成績の親戦略が見つからないか、ストレステストをパスできませんでした。"
                )
