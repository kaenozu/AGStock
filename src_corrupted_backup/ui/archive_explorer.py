...
#     """,
    ...
    extractor = KnowledgeExtractor()
        tab1, tab2, tab3, tab4 = st.tabs(["📖 決定の履歴", "🧠 知見の抽出", "🔮 予測の検証", "📊 統計分析"])
        with tab1:
...
for dec in reversed(decisions[-50:]):  # Latest 50
                with st.expander(f"{dec.get('timestamp', '')[:10]} | {dec.get('ticker')} | {dec.get('decision')}"):
        ...
        with tab2:
            ...
            if st.button("🧠 知見を抽出 (最新90日)"):
                with st.spinner("AIが過去の決定を分析中..."):
                    decisions = load_decisions_in_range(archive, datetime.now().replace(day=1), datetime.now())
                    if len(decisions) > 10:
                        insights = extractor.extract_universal_laws(decisions)
                        if insights.get("universal_laws"):
                            st.success(f"✅ {len(insights['universal_laws'])} 個の普遍的法則を発見しました")
...
#             st.markdown("""" パラダイム分布")
paradigm_df = pd.DataFrame(list(stats["paradigm_distribution"].items()), columns=["パラダイム", "件数"])
