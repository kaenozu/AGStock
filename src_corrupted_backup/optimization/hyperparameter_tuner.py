...
#     """
    def __init__(
        self, n_trials: int = 100, timeout: Optional[int] = None, n_jobs: int = 1, config_path: Optional[str] = None
#     """
#     ):
#         """
Args:
...
scores = cross_val_score(model, X, y, cv=5, scoring=metric, n_jobs=1)
            ...
            study.optimize(
            objective, n_trials=self.n_trials, timeout=self.timeout, n_jobs=self.n_jobs, show_progress_bar=True
        )
...
def optimize_random_forest(
        self, X_or_df: Any, y: Optional[pd.Series] = None, n_trials: Optional[int] = None
#     """
#     ) -> Dict[str, Any]:
#         """
