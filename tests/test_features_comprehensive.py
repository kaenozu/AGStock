...
from src.features import (
    add_technical_indicators, add_frequency_features,
    add_sentiment_features, add_advanced_features, add_macro_features
)
...
def test_add_sentiment_features(mock_sa_cls, sample_data):
    ...
    dates = pd.date_range(start='2023-01-01', periods=100, freq='D')
    history = [
        {'timestamp': d.isoformat(), 'score': np.random.uniform(-1, 1)}
        for d in dates
    ]
...
def test_add_macro_features_alignment(sample_data):
    ...
    macro_data = {
        'VIX': pd.DataFrame({'Close': np.random.uniform(10, 30, 100)}, index=macro_dates)
    }
