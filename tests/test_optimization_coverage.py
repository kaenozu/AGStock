...
from src.optimization import (HyperparameterOptimizer,
                              optimize_multi_objective, optimize_strategy_wfo)
...
class TestOptimization(unittest.TestCase):
    ...
    def test_optimize_random_forest(self):
        ...
        with patch("src.optimization.RandomForestClassifier") as MockRF, patch(
            "src.optimization.precision_score", return_value=0.8
    ...
    def test_optimize_lightgbm(self):
        ...
        # Mock lgb.train
        with patch("src.optimization.lgb.train") as mock_lgb_train, patch(
            "src.optimization.lgb.Dataset"
        ) as mock_lgb_dataset:
    ...
    def test_optimize_strategy_wfo(self):
        param_grid = {"param1": (10, 50), "param2": (0.1, 0.5)}

        # Run WFO
        result = optimize_strategy_wfo(self.df, MockStrategy, param_grid, window_size=100, step_size=50, n_trials=1)
    ...
    def test_optimize_multi_objective(self):
        param_grid = {"param1": (10, 50)}

        results = optimize_multi_objective(
            self.df, MockStrategy, param_grid, n_trials=2  # Need at least a few trials for pareto
        )

        self.assertIn(
            "pareto_solutions", results
        )  # Check actual key in source code if possible, assume 'pareto_solutions' or list returned?
