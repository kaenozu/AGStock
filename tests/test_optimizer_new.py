...
def test_objective_sma_crossover(sample_data):
    ...
    with patch.object(optimizer.backtester, "run") as mock_run:
        mock_run.return_value = {"total_trades": 10, "total_return": 15.5}
...
def test_objective_rsi_strategy(sample_data):
    ...
    with patch.object(optimizer.backtester, "run") as mock_run:
        mock_run.return_value = {"total_trades": 8, "total_return": 12.3}
...
def test_objective_bollinger_bands_strategy(sample_data):
    ...
    with patch.object(optimizer.backtester, "run") as mock_run:
        mock_run.return_value = {"total_trades": 12, "total_return": 18.7}
...
def test_objective_combined_strategy(sample_data):
    ...
    with patch.object(optimizer.backtester, "run") as mock_run:
        mock_run.return_value = {"total_trades": 15, "total_return": 20.5}
...
def test_objective_insufficient_trades(sample_data):
    ...
    with patch.object(optimizer.backtester, "run") as mock_run:
        # 取引数が3未満
        mock_run.return_value = {"total_trades": 2, "total_return": 5.0}
...
def test_optimize_logging_suppression(sample_data):
    ...
    with patch("src.optimizer.optuna.logging.set_verbosity") as mock_set_verbosity, patch(
        "src.optimizer.optuna.create_study"
