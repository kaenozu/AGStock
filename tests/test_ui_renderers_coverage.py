...
from src.ui_renderers import (
    render_performance_tab, render_paper_trading_tab, render_market_scan_tab,
    render_realtime_monitoring_tab, render_integrated_signal
)
...
class TestUIRenderers(unittest.TestCase):
    ...
    def test_render_performance_tab(self):
        ...
        with patch('src.ui_renderers.PerformanceAnalyzer') as MockAnalyzer:
            ...
            with patch('src.ui_renderers.pd.read_csv') as mock_read_csv: # Assuming it reads from CSV or DB?
                ...
                # Verify st calls
                self.assertTrue(self.mock_st.markdown.called or self.mock_st.header.called or self.mock_st.subheader.called)
    ...
    def test_render_market_scan_tab(self):
        ...
        with patch('src.backtester.Backtester') as MockBacktester, \
             patch('src.data_loader.fetch_stock_data') as mock_fetch, \
             patch('os.path.exists', return_value=False), \
             patch('src.sentiment.SentimentAnalyzer') as MockSA, \
             patch('src.data_loader.fetch_external_data') as mock_macro:
            ...
            mock_sa_instance.get_market_sentiment.return_value = {
                'score': 0.5, 'label': 'Neutral', 'news_count': 5, 'top_news': []
            }
