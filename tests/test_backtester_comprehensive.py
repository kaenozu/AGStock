...
def sample_data():
    ...
    df = pd.DataFrame(
        {"Open": prices, "High": prices + 2, "Low": prices - 2, "Close": prices, "Volume": 1000000}, index=dates
    )
...
def test_run_basic_short(sample_data):
    ...
    df = pd.DataFrame({"Open": prices, "High": prices + 1, "Low": prices - 1, "Close": prices}, index=dates)
...
def test_multi_asset(sample_data):
    ...
    data_map = {"Asset1": sample_data, "Asset2": sample_data.copy()}

    signals = [0] * 100
    signals[0] = 1
    signals[-2] = -1

    strategy_map = {"Asset1": MockStrategy(signals), "Asset2": MockStrategy(signals)}
