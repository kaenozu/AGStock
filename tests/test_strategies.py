...
class TestSMACrossoverStrategy:
    ...
    def test_golden_cross_generates_buy_signal(self):
        ...
        # 最初は下降、後半で上昇するパターン
        prices = np.concatenate([np.linspace(1000, 900, 25), np.linspace(900, 1100, 25)])  # 下降  # 上昇

        df = pd.DataFrame(
            {"Close": prices, "Open": prices, "High": prices + 5, "Low": prices - 5, "Volume": 1000000}, index=dates
        )
    ...
    def test_dead_cross_generates_sell_signal(self):
        ...
        # 最初は上昇、後半で下降するパターン
        prices = np.concatenate([np.linspace(900, 1100, 25), np.linspace(1100, 900, 25)])  # 上昇  # 下降
...
class TestRSIStrategy:
    ...
    def test_oversold_recovery_generates_buy_signal(self):
        ...
        # 急激な下落後に反発するパターン
        prices = np.concatenate([np.linspace(1000, 800, 20), np.linspace(800, 900, 30)])  # 急落  # 回復
...
class TestBollingerBandsStrategy:
    ...
    def test_lower_band_touch_generates_buy_signal(self):
        ...
        prices = np.concatenate(
            [np.full(30, 1000) + np.random.randn(30) * 5, np.linspace(1000, 900, 20)]  # 安定  # 急落
        )
