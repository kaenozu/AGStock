...
class TestExecutionEngineReal:
    ...
    def engine(self):
        ...
        mock_pt.get_current_balance.return_value = {
    ...
    def test_execute_orders_real_sell_skipped(self, engine):
        ...
        # ポジション保有状態をモック
        import pandas as pd
        engine.pt.get_positions.return_value = pd.DataFrame({
            'quantity': [100]
        }, index=['7203'])
    ...
    def test_check_risk_divergence(self, engine):
        """実残高と仮想残高の乖離チェック"""
        # 実残高が大きく異なる場合
        engine.real_broker.get_balance.return_value = {
            'total_equity': 500000 # 仮想は100万なので50%乖離
        }
