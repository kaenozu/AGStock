...
class TestDrawdownProtection:
    ...
    def test_no_loss_scenario(self, risk_manager, mock_paper_trader, mock_logger):
        ...
        equity_history = pd.DataFrame({
            'date': pd.date_range('2025-12-03', periods=2),
            'total_equity': [1000000, 1050000]
        })
        ...
        is_safe, reason, signals = risk_manager.check_drawdown_protection(
            mock_paper_trader, mock_logger
        )
...
class TestCorrelationCheck:
    ...
    def test_no_existing_positions(self, mock_fetch, risk_manager, mock_logger):
        """既存ポジションなし"""
        allow, reason = risk_manager.check_correlation(
            "AAPL", [], mock_logger
        )
    ...
    def test_low_correlation_allowed(self, mock_fetch, risk_manager, mock_logger):
        ...
        aapl_data = pd.DataFrame({
            'Close': 180 + np.random.randn(60) * 5
        }, index=dates)
        
        googl_data = pd.DataFrame({
            'Close': 140 + np.random.randn(60) * 3
        }, index=dates)
    ...
    def test_high_correlation_rejected(self, mock_fetch, risk_manager, mock_logger):
        ...
        mock_fetch.return_value = {
            'AAPL': aapl_data,
            'AAPL': aapl_data  # 同じデータ
        }
        
        allow, reason = risk_manager.check_correlation(
            "AAPL", ["AAPL"], mock_logger
        )
