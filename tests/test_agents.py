...
from src.agents import (AgentVote, FundamentalAnalyst, MacroStrategist,
                        PortfolioManager, RiskManager, TechnicalAnalyst)
...
class TestAgents(unittest.TestCase):
    def setUp(self):
        ...
        self.stock_df = pd.DataFrame(
            {"Close": np.random.rand(100) * 100 + 50, "Volume": np.random.rand(100) * 1000}, index=dates
        )
