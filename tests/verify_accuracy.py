...
def main():
    ...
    with open("report.md", "w", encoding="utf-8") as f:
        ...
        f.write("> [!IMPORTANT]\n")
        f.write(
            "> These results include **0.1% transaction costs** and **slippage**, and trades are executed at the **Next Day Open**.\n"
        )
        ...
        for _, row in top_5.iterrows():
            f.write(
                f"- **{row['Ticker']}** ({TICKER_NAMES.get(row['Ticker'], '')}) - {row['Strategy']}: Return {row['Total Return']*100:.1f}%, Win Rate {row['Win Rate']*100:.1f}%, Max DD {row['Max Drawdown']*100:.1f}% ({row['Total Trades']} trades)\n"
            )
