...
def optimize_threshold():
    ...
    if pred_values:
        print(f"予測値の統計: 最大={max(pred_values):.2f}%, 最小={min(pred_values):.2f}%, 平均={np.mean(pred_values):.2f}%")
    ...
    for threshold in thresholds:
        ...
        print(f"+{threshold:.1f}%     | {count:<10} | {win_rate:.1f}%     | {avg_return:+.2f}%     | {total_return:+.1f}%")
