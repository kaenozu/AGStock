...
def weekly_review():
    ...
    print_header(f"📈 週次パフォーマンスレビュー ({start_date.strftime('%Y-%m-%d')} 〜 {end_date.strftime('%Y-%m-%d')})")
    ...
    print(f"含み損益:      {format_currency(balance['unrealized_pnl'])} ({format_percentage(balance['unrealized_pnl'] / balance['invested_amount'] if balance['invested_amount'] > 0 else 0)})")
    ...
    if positions.empty:
        ...
        for idx, pos in positions_sorted.head(10).iterrows():
            ...
            print(f"{ticker:<12} {qty:>8} {format_currency(entry):>12} {format_currency(current):>12} {pnl_str}{format_currency(pnl):>13} {format_percentage(pnl_pct/100):>10}")
        ...
        if not recent.empty:
            ...
            for _, row in recent.iterrows():
                ...
                if prev_equity is not None:
                    ...
                    change_str = f"{'+' if change >= 0 else ''}{format_currency(change)} ({format_percentage(change_pct)})"
                else:
                    change_str = "-"
